...
--without-pear --with-gettext --with-mime-magic --enable-exif --with-snmp=/usr/local/snmp --enable-calendar
...
if grep -q "autoconf-2.68" testbuildconf; 
then 
export PHP_AUTOCONF=/usr/local/autoconf-2.68/bin/autoconf
export PHP_AUTOHEADER=/usr/local/autoconf-2.68/bin/autoheader
./buildconf --force
else 
